...
namespace Gerenciadores
{
    ...
    void Subject::handleKeyPressed(sf::Keyboard::Key key) {
        for (it = lista_observers.begin(); it != lista_observers.end(); ++it)
            (*it)->notifyPressed(getKeyasString(key));
    }

    /* Check for keys pressed and notify every Observer. */
    void Subject::handleKeyReleased(sf::Keyboard::Key key) {
        for (it = lista_observers.begin(); it != lista_observers.end(); ++it)
            (*it)->notifyReleased(getKeyasString(key));
    }

    std::string Subject::getKeyasString(sf::Keyboard::Key key) {
